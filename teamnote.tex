...
vector<int> berlekamp_massey(vector<int> x){
	...
	for(int i=0; i<x.size(); i++){
		...
		vector<int> c(i-lf-1); 
		c.push_back(k);
		for(auto &j : ls) c.push_back(-j * k % mod);5
...
struct elem{int x, y, v;}; // A_(x, y) <- v, no duplicate please

lint det(int n, vector<elem> v){
	if(n == 0) return 1;
	mt19937 rng(0x14004);
	...
	vector<int> c, l, r;
	for(int i=0; i<n; i++){
		c.push_back(randint(1, mod - 1));
		l.push_back(randint(1, mod - 1));
		r.push_back(randint(1, mod - 1));
	}
	vector<int> buf(2*n);
	for(int i=0; i<2*n; i++){
		for(int j=0; j<n; j++){
			buf[i] += 1ll * l[j] * r[j] % mod;
			if(buf[i] >= mod) buf[i] -= mod;
		}
		for(int j=0; j<n; j++) r[j] = 1ll * r[j] * c[j] % mod;
		vector<int> gob(n);
		for(auto &j : v){
			gob[j.x] += 1ll * r[j.y] * j.v % mod;
			if(gob[j.x] < 0) gob[j.x] += mod;
			if(gob[j.x] >= mod) gob[j.x] -= mod;
		}
		r = gob;
	}
	auto ret = berlekamp_massey(buf);
	if(ret.empty() || ret.back() == 0) return 0;
	assert(ret.size() == n);
	if(ret.size() != n) return det(n, v);
	lint cdet = 1;
	for(int i=0; i<n; i++) cdet *= c[i], cdet %= mod;
	if(n % 2 == 0) ret.back() = mod - ret.back();
	return ret.back() * ipow(cdet, mod - 2) % mod;
}
\end{minted}
...
\begin{itemize}
	...
	\item LP Duality. max $c^Tx$ sjt to $Ax \leq b$. Dual problem is min $b^Tx$ sjt to $A^Tx \leq c$. By strong duality, min max value coincides.
\end{itemize}
